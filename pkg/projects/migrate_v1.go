...
func v1ProjectMarker(srcOwner string, srcProjectNumber int) string {
	projectKey := fmt.Sprintf("v1:%s#%d", srcOwner, srcProjectNumber)
...
func v1ItemMarker(srcOwner string, srcProjectNumber int, cardID int64) string {
	projectKey := fmt.Sprintf("v1:%s#%d", srcOwner, srcProjectNumber)
	projectHash := fmt.Sprintf("%x", sha256.Sum256([]byte(projectKey)))[:16]
...
func MigrateProjectV1ToV2(ctx context.Context, src, dst *gh.GitHubClient, srcOwner, srcRepoName, dstOwner string, srcNumber int, opts *MigrateV1Options) (*gh.ProjectV2, error) {
	...
	marker := v1ProjectMarker(srcOwner, srcNumber)
	...
	if opts != nil && opts.Prune {
		...
		return migrateV1ItemsInto(ctx, src, dst, srcOwner, dstOwner, srcProject.ID, srcNumber, srcColumns, cardsByColumn, prev, marker, opts)
	}
	...
	return migrateV1ItemsInto(ctx, src, dst, srcOwner, dstOwner, srcProject.ID, srcNumber, srcColumns, cardsByColumn, dstProject, marker, opts)
}

// migrateV1ItemsInto migrates all v1 cards into an existing v2 destination project.
func migrateV1ItemsInto(ctx context.Context, src, dst *gh.GitHubClient, srcOwner, dstOwner string, srcProjectID int64, srcNumber int, srcColumns []gh.ProjectV1Column, cardsByColumn [][]gh.ProjectV1Card, dstProject *gh.ProjectV2, marker string, opts *MigrateV1Options) (*gh.ProjectV2, error) {
	...
	for i, col := range srcColumns {
		...
		for _, card := range cardsByColumn[i] {
			itemMarker := v1ItemMarker(srcOwner, srcNumber, card.ID)

			// Skip already-migrated items unless overwrite is requested.
			if opts == nil || !opts.Overwrite {
				if findItemByMarker(existingItems, itemMarker) != nil {
